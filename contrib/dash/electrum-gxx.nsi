...
;Variables
  ...
  !define PREV_PROD_NAME "Electrum-GXX"
  !define PREV_PROD_NAME2 "Dash-Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/akhavr/electrum-gxx"
...
;Interface Settings
  ...
  !define MUI_ICON "electrum_gxx\gui\icons\electrum-gxx.ico"
...
Section "${PRODUCT_NAME}" SectionDE
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\gxx"
  ...
  File /r "dist\electrum-gxx\*.*"
  File "electrum_gxx\gui\icons\electrum-gxx.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-gxx-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-gxx-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-gxx-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-gxx-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-gxx-${PRODUCT_VERSION}.exe" 0


  ;Links gxx: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\gxx" "" "URL:gxx Protocol"
  WriteRegStr HKCU "Software\Classes\gxx" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\gxx" "DefaultIcon" "$\"$INSTDIR\electrum-gxx.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\gxx\shell\open\command" "" "$\"$INSTDIR\electrum-gxx-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-gxx.ico"
